 ...
 Ext.define('app.controller.ProductStock',{
		...
		init : function(application) {
			...
			me.control({
				...
				'ProductStock_List>toolbar button[action=add]':{
					click : this.add
				},
				'ProductStock_List>toolbar button[action=edit]' : {
					click : this.edit
				},
				'ProductStock_List>toolbar button[action=del]':{
					click : this.del
				},
				'ProductStock_Add  button[action=save]':{
					click:this.save_add
				},
				'ProductStock_Edit  button[action=save]':{
		...
		add : function(btn){
			//用于新增数据的方法
			var window_id='ProductStock_add_window';
			...
			*/
			...
			var new_record = Ext.create('app.model.ProductStock',{
				//default data
			});

			var _view=Ext.widget('ProductStock_Add');
		...
		save_add:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/productstock/add",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProductstock_gridpanel().getStore().reload();
							add_window.close();
						}else{
		...
		del : function(btn){
			...
			Ext.MessageBox.confirm("提示","您确定要删除这些信息吗?",function(button,text){ 
				if (button=='yes'){
					Ext.Ajax.request({
						url: GLOBAL_ROOT_PATH+'/productstock/del',
						params: {
		...
		//打开编辑窗口并加载待编辑数据
		edit : function(btn){
			var window_id='ProductStock_edit_window';
			...
			*/
			...
			var _view=Ext.widget('ProductStock_Edit');
		...
		save_edit:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/productstock/edit",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProductstock_gridpanel().getStore().reload();
							me.getProductstock_gridpanel().getSelectionModel().deselectAll();
							edit_window.close();
						}else{
							Ext.Msg.alert('错误', action.result.msg);
							form.markInvalid(action.result.errors);
