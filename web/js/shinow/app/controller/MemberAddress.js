/**
 * <p>
 * 会员收货地址信息表 controller
 * </p>
 * 
 * @author 代码生成器
 * @class app.controller.MemberAddress
 * @extends Ext.app.Controller
 */
 Ext.define('app.controller.MemberAddress',{
		extend : 'Ext.app.Controller',
		alias : 'controller.MemberAddress',
		requires : [ 'app.model.MemberAddress', 'app.store.MemberAddress','app.view.memberaddress.MemberAddress_List','app.view.memberaddress.MemberAddress_Add','app.view.memberaddress.MemberAddress_Edit' ],
		models: ['MemberAddress'],
		views : ['memberaddress.MemberAddress_List','memberaddress.MemberAddress_Add','memberaddress.MemberAddress_Edit'],
		stores : ['MemberAddress'],
		refs : [
			{
				ref : 'memberaddress_gridpanel',
				selector : 'MemberAddress_List>gridpanel'
			}
		],
		init : function(application) {
			var me = this;
			me.control({
				'MemberAddress_List>gridpanel':{
					render : this.loadDefaultListData
				},
				'MemberAddress_List>toolbar button[action=add]':{
					click : this.add
				},
				'MemberAddress_List>toolbar button[action=edit]' : {
					click : this.edit
				},
				'MemberAddress_List>toolbar button[action=del]':{
					click : this.del
				},
				'MemberAddress_Add  button[action=save]':{
					click:this.save_add
				},
				'MemberAddress_Edit  button[action=save]':{
		...
		add : function(btn){
			//用于新增数据的方法
			var window_id='MemberAddress_add_window';
			...
			*/
			...
			var new_record = Ext.create('app.model.MemberAddress',{
				//default data
			});

			var _view=Ext.widget('MemberAddress_Add');
		...
		save_add:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/memberaddress/add",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getMemberaddress_gridpanel().getStore().reload();
							add_window.close();
						}else{
		...
		del : function(btn){
			...
			Ext.MessageBox.confirm("提示","您确定要删除这些信息吗?",function(button,text){ 
				if (button=='yes'){
					Ext.Ajax.request({
						url: GLOBAL_ROOT_PATH+'/memberaddress/del',
						params: {
							id: rec.get('id')
						},
		...
		//打开编辑窗口并加载待编辑数据
		edit : function(btn){
			var window_id='MemberAddress_edit_window';
			...
			*/
			...
			var _view=Ext.widget('MemberAddress_Edit');
		...
		save_edit:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/memberaddress/edit",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getMemberaddress_gridpanel().getStore().reload();
							me.getMemberaddress_gridpanel().getSelectionModel().deselectAll();
							edit_window.close();
						}else{
							Ext.Msg.alert('错误', action.result.msg);
							form.markInvalid(action.result.errors);
