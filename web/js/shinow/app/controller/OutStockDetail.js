 ...
 * 
 * @author 代码生成器
 * @class app.controller.OutStockDetail
 * @extends Ext.app.Controller
 */
 Ext.define('app.controller.OutStockDetail',{
		extend : 'Ext.app.Controller',
		alias : 'controller.OutStockDetail',
		requires : [ 'app.model.OutStockDetail', 'app.store.OutStockDetail','app.view.outstockdetail.OutStockDetail_List','app.view.outstockdetail.OutStockDetail_Add','app.view.outstockdetail.OutStockDetail_Edit' ],
		models: ['OutStockDetail'],
		views : ['outstockdetail.OutStockDetail_List','outstockdetail.OutStockDetail_Add','outstockdetail.OutStockDetail_Edit'],
		stores : ['OutStockDetail'],
		refs : [
			{
				ref : 'outstockdetail_gridpanel',
				selector : 'OutStockDetail_List>gridpanel'
			}
		],
		init : function(application) {
			var me = this;
			me.control({
				'OutStockDetail_List>gridpanel':{
					render : this.loadDefaultListData
				},
				'OutStockDetail_List>toolbar button[action=add]':{
					click : this.add
				},
				'OutStockDetail_List>toolbar button[action=edit]' : {
					click : this.edit
				},
				'OutStockDetail_List>toolbar button[action=del]':{
					click : this.del
				},
				'OutStockDetail_Add  button[action=save]':{
					click:this.save_add
				},
				'OutStockDetail_Edit  button[action=save]':{
		...
		add : function(btn){
			//用于新增数据的方法
			var window_id='OutStockDetail_add_window';
			...
			*/
			...
			var new_record = Ext.create('app.model.OutStockDetail',{
				//default data
			});

			var _view=Ext.widget('OutStockDetail_Add');
		...
		save_add:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/outstockdetail/add",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getOutstockdetail_gridpanel().getStore().reload();
							add_window.close();
						}else{
		...
		del : function(btn){
			...
			Ext.MessageBox.confirm("提示","您确定要删除这些信息吗?",function(button,text){ 
				if (button=='yes'){
					Ext.Ajax.request({
						url: GLOBAL_ROOT_PATH+'/outstockdetail/del',
						params: {
							id: rec.get('outStockDetailId')
						},
		...
		//打开编辑窗口并加载待编辑数据
		edit : function(btn){
			var window_id='OutStockDetail_edit_window';
			...
			*/
			...
			var _view=Ext.widget('OutStockDetail_Edit');
		...
		save_edit:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/outstockdetail/edit",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getOutstockdetail_gridpanel().getStore().reload();
							me.getOutstockdetail_gridpanel().getSelectionModel().deselectAll();
							edit_window.close();
						}else{
							Ext.Msg.alert('错误', action.result.msg);
							form.markInvalid(action.result.errors);
