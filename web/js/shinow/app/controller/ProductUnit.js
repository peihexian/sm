 ...
 * 
 * @author 代码生成器
 * @class app.controller.ProductUnit
 * @extends Ext.app.Controller
 */
 Ext.define('app.controller.ProductUnit',{
		extend : 'Ext.app.Controller',
		alias : 'controller.ProductUnit',
		requires : [ 'app.model.ProductUnit', 'app.store.ProductUnit','app.view.productunit.ProductUnit_List','app.view.productunit.ProductUnit_Add','app.view.productunit.ProductUnit_Edit' ],
		models: ['ProductUnit'],
		views : ['productunit.ProductUnit_List','productunit.ProductUnit_Add','productunit.ProductUnit_Edit'],
		stores : ['ProductUnit'],
		refs : [
			{
				ref : 'productunit_gridpanel',
				selector : 'ProductUnit_List>gridpanel'
			}
		],
		init : function(application) {
			var me = this;
			me.control({
				'ProductUnit_List>gridpanel':{
					render : this.loadDefaultListData
				},
				'ProductUnit_List>toolbar button[action=add]':{
					click : this.add
				},
				'ProductUnit_List>toolbar button[action=edit]' : {
					click : this.edit
				},
				'ProductUnit_List>toolbar button[action=del]':{
					click : this.del
				},
				'ProductUnit_Add  button[action=save]':{
					click:this.save_add
				},
				'ProductUnit_Edit  button[action=save]':{
		...
		add : function(btn){
			//用于新增数据的方法
			var window_id='ProductUnit_add_window';
			...
			*/
			...
			var new_record = Ext.create('app.model.ProductUnit',{
				//default data
			});

			var _view=Ext.widget('ProductUnit_Add');
		...
		save_add:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/productunit/add",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProductunit_gridpanel().getStore().reload();
							add_window.close();
						}else{
		...
		del : function(btn){
			...
			Ext.MessageBox.confirm("提示","您确定要删除这些信息吗?",function(button,text){ 
				if (button=='yes'){
					Ext.Ajax.request({
						url: GLOBAL_ROOT_PATH+'/productunit/del',
						params: {
							id: rec.get('unitId')
						},
		...
		//打开编辑窗口并加载待编辑数据
		edit : function(btn){
			var window_id='ProductUnit_edit_window';
			...
			*/
			...
			var _view=Ext.widget('ProductUnit_Edit');
		...
		save_edit:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/productunit/edit",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProductunit_gridpanel().getStore().reload();
							me.getProductunit_gridpanel().getSelectionModel().deselectAll();
							edit_window.close();
						}else{
							Ext.Msg.alert('错误', action.result.msg);
							form.markInvalid(action.result.errors);
