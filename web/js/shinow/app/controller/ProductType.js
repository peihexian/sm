 ...
 * 
 * @author 代码生成器
 * @class app.controller.ProductType
 * @extends Ext.app.Controller
 */
 Ext.define('app.controller.ProductType',{
		extend : 'Ext.app.Controller',
		alias : 'controller.ProductType',
		requires : [ 'app.model.ProductType', 'app.store.ProductType','app.view.producttype.ProductType_List','app.view.producttype.ProductType_Add','app.view.producttype.ProductType_Edit' ],
		models: ['ProductType'],
		views : ['producttype.ProductType_List','producttype.ProductType_Add','producttype.ProductType_Edit'],
		stores : ['ProductType'],
		refs : [
			{
				ref : 'producttype_gridpanel',
				selector : 'ProductType_List>gridpanel'
			}
		],
		init : function(application) {
			var me = this;
			me.control({
				'ProductType_List>gridpanel':{
					render : this.loadDefaultListData
				},
				'ProductType_List>toolbar button[action=add]':{
					click : this.add
				},
				'ProductType_List>toolbar button[action=edit]' : {
					click : this.edit
				},
				'ProductType_List>toolbar button[action=del]':{
					click : this.del
				},
				'ProductType_Add  button[action=save]':{
					click:this.save_add
				},
				'ProductType_Edit  button[action=save]':{
		...
		add : function(btn){
			//用于新增数据的方法
			var window_id='ProductType_add_window';
			...
			*/
			...
			var new_record = Ext.create('app.model.ProductType',{
				//default data
			});

			var _view=Ext.widget('ProductType_Add');
		...
		save_add:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/producttype/add",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProducttype_gridpanel().getStore().reload();
							add_window.close();
						}else{
		...
		del : function(btn){
			...
			Ext.MessageBox.confirm("提示","您确定要删除这些信息吗?",function(button,text){ 
				if (button=='yes'){
					Ext.Ajax.request({
						url: GLOBAL_ROOT_PATH+'/producttype/del',
						params: {
							id: rec.get('typeCode')
						},
		...
		//打开编辑窗口并加载待编辑数据
		edit : function(btn){
			var window_id='ProductType_edit_window';
			...
			*/
			...
			var _view=Ext.widget('ProductType_Edit');
		...
		save_edit:function(btn){
			...
			if (form.isValid()) {
				form.submit({
					url:GLOBAL_ROOT_PATH+"/producttype/edit",
					waitTitle : '提示',
					waitMsg : '正在提交数据,请稍候...',
					success: function(form, action) {
						if (true==action.result.success){
							Ext.Msg.alert('成功', action.result.msg);
							me.getProducttype_gridpanel().getStore().reload();
							me.getProducttype_gridpanel().getSelectionModel().deselectAll();
							edit_window.close();
						}else{
							Ext.Msg.alert('错误', action.result.msg);
							form.markInvalid(action.result.errors);
